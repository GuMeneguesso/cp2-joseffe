...
public class Sistema {
	...
	public static void main(String[] args) {
		...
		for(int i=0; i<5; i++) {
			System.out.printf("\n %s \n", listaCliente[i].exibirNomeIdade());
			System.out.printf("\n %s \n", listaCliente[i].exibirDadosConta());
			
			System.out.printf("Deseja atualizar o email? (S/N)");
			atualizaEmail = ler.next().charAt(0);
			
			if (atualizaEmail == 'S') {
				System.out.printf("Diite o email do cliente: ");
			    email = ler.next();
			    
			    listaCliente[i].atualizarEmail(email);
			    System.out.printf("Email Atualizado com sucesso! (%s)", listaCliente[i].email);
			}
		}
